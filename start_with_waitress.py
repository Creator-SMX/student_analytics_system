...
# -*- coding: utf-8 -*-
"""
使用Waitress WSGI服务器启动应用
Waitress是一个纯Python实现的WSGI服务器，适合在Windows环境下运行
"""
...
def main():
    try:
        logger.info("开始启动学生消费分析系统 (使用Waitress WSGI服务器)...")
        ...
        # 使用Waitress启动应用
        logger.info("使用Waitress WSGI服务器启动应用...")
        ...
        serve(
            ...
            host='0.0.0.0',
            port=5001,
            threads=4,  # 使用4个线程处理请求
            connection_limit=1000,  # 最大连接数
            channel_timeout=300  # 连接超时时间(秒)
        )
